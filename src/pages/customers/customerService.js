...
export async function listCustomers(q = "", page = 1, pageSize = 25) {
  ...
  if (term) {
    if (isNum) {
      const like = `%${term}%`;
      where =
        "WHERE (C.customer_id = ? OR C.first_name LIKE ? OR C.last_name LIKE ? OR CONCAT(C.first_name,' ',C.last_name) LIKE ?)";
      params = [Number(term), like, like, like];
    } else {
      const clauses = tokens.map(
        () =>
          "(C.first_name LIKE ? OR C.last_name LIKE ? OR CONCAT(C.first_name,' ',C.last_name) LIKE ?)"
      );
